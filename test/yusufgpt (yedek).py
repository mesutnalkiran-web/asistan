...
async def speak(text):
    ...
    # Metni ElevenLabs için temizle (işaretleri metne çevirir)
        ...
        try:
            ...
            if os.path.exists(bat_path):
                ...
                if cap <= 20 and stat == "Discharging" and not batarya_uyarisi_verildi:
                    asyncio.run(speak(f"Mesut usta, şarjım yüzde {cap} seviyesine düştü. Kapanmak üzereyim, beni fişe takar mısın?"))
                    batarya_uyarisi_verildi = True
                elif stat == "Charging": batarya_uyarisi_verildi = False
        except: pass
        time.sleep(300)
...
def listen_and_process(duration=4):
    r = sr.Recognizer(); audio_file = "input_audio.wav"
    try:
...
def check_battery():
    try:
        ...
        if os.path.exists(bat_path):
            ...
            with open(f"{bat_path}/status", "r") as f: stat = f.read().strip()
            durum = "şarj oluyor" if stat == "Charging" else "pilde çalışıyorum"
            return f"Şu an yüzde {cap} şarjım var ve {durum} usta."
        return "Batarya sensörümü okuyamıyorum."
    except: return "Batarya durumuna şu an bakamıyorum."
